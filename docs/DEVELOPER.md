...
`aisctx_ai_connector_configured` — return a boolean to declare whether a usable AI
connector exists. Return `null` (the default) to let the plugin decide.

```php
add_filter('aisctx_ai_connector_configured', function ($configured) {
...
`aisctx_generate_document` — return a non-empty Markdown string to supply or
override the generated document.

```php
add_filter('aisctx_generate_document', function ($document, array $payload, array $context) {
...
- If the `aisctx_provider_id` filter returns a non-empty provider ID, that provider is used.
- Else, if `aisctx_model_id` maps to a registered connector ID, that provider is used.
...
- `aisctx_ai_connector_configured` (mixed) — override connector detection.
- `aisctx_generate_document` (`$document`, `array $payload`, `array $context`) — supply or override the generated Markdown.
- `aisctx_provider_id` (string) — force a specific provider ID.
- `aisctx_model_id` (string) — map a model ID to a registered connector.
- `aisctx_exit_after_redirect` (bool) — control `exit()` after admin redirects (useful in tests).

Constants:

- `AISCTX_DISABLE_BOOTSTRAP` — when `true`, prevents the plugin from bootstrapping (used by unit tests).
