...
final class AISCTX_Plugin_Test extends WP_UnitTestCase {
    public function test_rewrite_rule_registered_for_llms_md(): void {
        ...
        $this->assertSame('index.php?aisctx=1', $rules['llms\\.md$']);
    }
    ...
    public function test_query_var_is_registered(): void {
        ...
        $this->assertContains('aisctx', $query_vars, 'Expected aisctx query var to be registered.');
    }

    public function test_connector_gate_filter_can_force_missing_connector_state(): void {
        add_filter('aisctx_ai_connector_configured', '__return_false');

        $plugin = new ReflectionClass('AISCTX_Plugin');
        ...
        remove_filter('aisctx_ai_connector_configured', '__return_false');
    }
    ...
    public function test_preview_payload_admin_action_updates_option_and_redirects(): void {
        ...
        $captured_redirect = '';
        add_filter('aisctx_exit_after_redirect', '__return_false');
        ...
        $_REQUEST['_wpnonce'] = wp_create_nonce('aisctx_preview_payload');

        do_action('admin_post_aisctx_preview_payload');

        $preview = get_option('aisctx_admin_payload_preview', '');
        ...
        $this->assertStringContainsString('aisctx_notice=preview_ready', $captured_redirect);

        unset($_REQUEST['_wpnonce']);
        remove_filter('aisctx_exit_after_redirect', '__return_false');
    ...
    public function test_request_uri_matcher_accepts_subdirectory_multisite_double_slash_path(): void {
        $plugin_ref = new ReflectionClass('AISCTX_Plugin');
